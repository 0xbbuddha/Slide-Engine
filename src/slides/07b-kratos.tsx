...
export const meta: SlideMeta = {
  title: 'Kratos',
  speaker: ['Harouna'],
  notes: 'Kratos est le seul agent qui utilise des direct syscalls via Hell\'s Gate. Montrez l\'animation de resolution SSN. Insistez sur le fait que ntdll est scannee dynamiquement - aucun SSN hardcode.',
}

// ── Hell's Gate SSN resolution animation ──────────────────────────────────
...
// Syscall stub pattern: mov r10,rcx | mov eax,SSN | syscall
function makeStub(ssn: number) {
  return [0x4C, 0x8B, 0xD1, 0xB8, ssn, 0x00, 0x00, 0x00, 0x0F, 0x05]
}

// Random noise bytes around the stub
...
function HellsGate() {
  ...
  // Layout: 4 noise | stub (10) | 4 noise = 18 bytes total
  ...
  return (
    <div style={{ fontFamily: mono, userSelect: 'none' }}>
      <div style={{ fontSize: tokens.type.size.xs, color: tokens.color.text.muted, marginBottom: 8, letterSpacing: '0.08em' }}>
        Hell's Gate - resolution SSN depuis ntdll.dll
      </div>
      ...
      <AnimatePresence mode="wait">
        <motion.div
          ...
          style={{ fontSize: tokens.type.size.xs, color: B, fontWeight: 700, marginBottom: 10, letterSpacing: '0.02em' }}
        >
      ...
      <div style={{ display: 'flex', gap: 3, flexWrap: 'wrap', marginBottom: 10 }}>
        {allBytes.map((b, i) => {
          ...
          const bg = isActive
            ? `${B}20`
            : isStub && isScanned
              ? phase === 'done' ? `${B}12` : `${B}18`
              : tokens.color.surface.tech

          const border = isActive
            ? `${B}90`
            : isStub && isScanned
              ? `${B}50`
              : tokens.color.surface.line

          const textColor = isStub && isScanned
            ? phase === 'done' && stubOff === 4 ? tokens.color.semantic.success : B
          ...
          return (
            ...
            >
              <div style={{ textAlign: 'center', padding: '4px 2px', fontSize: tokens.type.size['2xs'], fontWeight: isStub && isScanned ? 700 : 400, color: textColor, transition: 'color 0.15s' }}>
                {b.toString(16).padStart(2, '0').toUpperCase()}
              </div>
              <div style={{ textAlign: 'center', fontSize: '7px', color: isStub && isScanned ? B : 'transparent', padding: '1px 0 3px', lineHeight: 1, letterSpacing: 0 }}>
      ...
      <AnimatePresence mode="wait">
        {phase === 'done' && (
          ...
          >
            <span style={{ fontSize: tokens.type.size.xs, color: tokens.color.text.muted }}>SSN resolu</span>
            <span style={{ fontSize: '13px', fontWeight: 700, color: tokens.color.semantic.success }}>
              0x{sc.ssn.toString(16).padStart(2, '0').toUpperCase()}
            </span>
            <span style={{ fontSize: tokens.type.size.xs, color: tokens.color.text.muted, flex: 1, textAlign: 'right' }}>{sc.name}</span>
          </motion.div>
        )}
        {phase !== 'done' && (
          <motion.div
          ...
          >
            ...
            />
            <span style={{ fontSize: tokens.type.size.xs, color: tokens.color.text.muted }}>scan ntdll.dll...</span>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <div style={{ display: 'flex', gap: 14, marginTop: 8 }}>
        {[
          { label: 'bruit', color: tokens.color.text.muted },
          { label: 'stub syscall', color: B },
          { label: 'SSN extrait', color: tokens.color.semantic.success },
        ].map(({ label, color }) => (
          <div key={label} style={{ fontSize: tokens.type.size['2xs'], color, letterSpacing: '0.06em', fontWeight: 600 }}>{label.toUpperCase()}</div>
        ))}
      </div>
    </div>
  )
...
const EVASION = [
  ...
  'ETW/AMSI patch en memoire',
  'Direct syscalls via Hell\'s Gate (SSN dynamique)',
  'PPID spoofing - parent process usurpe',
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16, width: '100%', alignItems: 'flex-start' }}>
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
        <Eyebrow>07b - Kratos - Focus C99</Eyebrow>
      </motion.div>
      ...
      >
        <span style={{ fontFamily: tokens.type.family.mono, fontSize: '28px', fontWeight: 700, color: B, letterSpacing: '-0.04em', lineHeight: 1 }}>C99</span>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1.1fr', gap: 18, width: '100%' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          >
            <KratosLogo />
            <div style={{ flex: 1 }}>
              <div style={{ fontFamily: tokens.type.family.mono, fontSize: '22px', fontWeight: 700, color: B, letterSpacing: '-0.03em', lineHeight: 1, marginBottom: 6 }}>Kratos</div>
              <div style={{ fontSize: tokens.type.size.xs, color: tokens.color.text.muted, lineHeight: tokens.type.leading.relaxed }}>
                Agent C99 minimaliste pour Windows x64. Architecture modulaire compilee conditionnellement - chaque commande est un module independant.
              </div>
              <div style={{ display: 'flex', gap: 6, marginTop: 10, flexWrap: 'wrap' }}>
                <Tag color={B}>C99</Tag>
                <Tag color={tokens.color.text.muted}>Windows x64</Tag>
                <Tag color={tokens.color.text.muted}>Hell's Gate</Tag>
              </div>
            </div>
          </motion.div>
          ...
          >
            ...
            >
              {EVASION.map(item => (
                ...
                >
                  <span style={{ color: B, flexShrink: 0, fontFamily: tokens.type.family.mono, fontSize: tokens.type.size['2xs'], fontWeight: 700 }}>→</span>
                  {item}
                </motion.div>
              ))}
            </motion.div>
          ...
          >
            ...
            <div style={{ display: 'flex', gap: 12, marginTop: 8, flexWrap: 'wrap' }}>
              {POSTEX.map(({ label, items }) => (
                <div key={label}>
                  <div style={{ fontSize: tokens.type.size['2xs'], fontFamily: tokens.type.family.mono, color: tokens.color.text.muted, fontWeight: 600, letterSpacing: '0.06em', textTransform: 'uppercase', marginBottom: 4 }}>{label}</div>
                  <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
                    {items.map(cmd => (
                      <span key={cmd} style={{ fontFamily: tokens.type.family.mono, fontSize: tokens.type.size['2xs'], color: B, background: `${B}10`, border: `1px solid ${B}30`, borderRadius: 4, padding: '2px 6px' }}>{cmd}</span>
                    ))}
                  </div>
                </div>
              ))}
        ...
        {/* Right - Hell's Gate animation */}
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          <motion.div
            initial={{ opacity: 0, x: 12 }}
            animate={{ opacity: 1, x: 0 }}
            transition={{ duration: 0.5, delay: 0.25 }}
            style={{ background: tokens.color.surface.tech, border: `1px solid ${tokens.color.surface.line}`, borderLeft: `2px solid ${B}`, borderRadius: 10, padding: '16px 18px', flex: 1 }}
          >
            <HellsGate />
          </motion.div>
          ...
          >
            <p style={{ fontSize: tokens.type.size.sm, color: tokens.color.text.secondary, margin: 0, lineHeight: tokens.type.leading.normal }}>
              Aucun SSN hardcode. Le stub syscall est resolu dynamiquement depuis ntdll.dll - contourne les hooks userland sans patcher la memoire de l'EDR.
            </p>
          </motion.div>
        </div>
